...
# -----------------------------
# Fetch metadata-protected builds
# -----------------------------
...
while read -r path; do
  ...
  if curl -sf --connect-timeout 5 --max-time 10 \
    -u "$ART_OSS_USER:$ART_OSS_PASSWORD" "$meta_url" -o meta.xml; then
...
END {
  for (p in build_count) {
    ...
    # Handle case where total builds <= keep
    else {
      for (i=1; i<=build_count[p]; i++) {
        build=build_order[p,i]
        key=p "|" build
